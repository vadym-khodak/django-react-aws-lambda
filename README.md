...
```bash
pip install django
django-admin startproject django_react_aws_lambda
```
...
```python
"""
Django settings for django_react_aws_lambda project.
...
ROOT_URLCONF = 'django_react_aws_lambda.urls'
...
WSGI_APPLICATION = 'django_react_aws_lambda.wsgi.application'
...
```bash
export DJANGO_SETTINGS_MODULE=django_react_aws_lambda.local
```
...
custom:
  ...
  stage: ${env:STAGE}
  wsgi:
    app: django_react_aws_lambda.wsgi.application
